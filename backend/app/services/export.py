...
async def create_order_zip(order: Order, session: Session) -> io.BytesIO:
    ...
    if not order_items:
        raise ValueError("주문에 책이 포함되어 있지 않습니다")
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        ...
        for item in order_items:
            ...
            books_path = f"books/{book_filename}"
            zip_file.writestr(books_path, book_json_bytes)
            manifest_entries.append({
                "path": books_path,
                "sha256": sha256(book_json_bytes).hexdigest(),
                "size": len(book_json_bytes)
            })
